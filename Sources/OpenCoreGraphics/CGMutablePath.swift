...
public class CGMutablePath: CGPath, @unchecked Sendable {
    ...
    public func addArc(center: CGPoint, radius: CGFloat, startAngle: CGFloat, endAngle: CGFloat,
        ...
        let startPoint = CGPoint(
            x: center.x + radius * CGFloat(cos(startAngle.native)),
            y: center.y + radius * CGFloat(sin(startAngle.native))
        ).applying(transform)
        ...
        var angle = startAngle.native
        let targetAngle = endAngle.native
        let direction = clockwise ? -1.0 : 1.0
        let step = Double.pi / 2 * direction
        ...
        while (clockwise ? angle > targetAngle : angle < targetAngle) {
            ...
            addArcSegment(center: center, radius: radius, startAngle: CGFloat(angle),
                         endAngle: CGFloat(nextAngle), transform: transform)
            angle = nextAngle
        }
    }
    ...
    private func addArcSegment(center: CGPoint, radius: CGFloat, startAngle: CGFloat,
        ...
        let angleDiff = (endAngle - startAngle).native
        ...
        let p3 = CGPoint(
            x: center.x + radius * CGFloat(cos(endAngle.native)),
            y: center.y + radius * CGFloat(sin(endAngle.native))
        )
        let p1 = CGPoint(
            x: p0.x - k * radius * CGFloat(sin(startAngle.native)),
            y: p0.y + k * radius * CGFloat(cos(startAngle.native))
        )
        let p2 = CGPoint(
            x: p3.x + k * radius * CGFloat(sin(endAngle.native)),
            y: p3.y - k * radius * CGFloat(cos(endAngle.native))
        )
    ...
    public func addArc(tangent1End: CGPoint, tangent2End: CGPoint, radius: CGFloat,
        ...
        // Normalize vectors
        let len1 = sqrt(v1.x.native * v1.x.native + v1.y.native * v1.y.native)
        let len2 = sqrt(v2.x.native * v2.x.native + v2.y.native * v2.y.native)
        ...
        let n1 = CGPoint(x: CGFloat(v1.x.native / len1), y: CGFloat(v1.y.native / len1))
        let n2 = CGPoint(x: CGFloat(v2.x.native / len2), y: CGFloat(v2.y.native / len2))

        // Calculate angle between vectors
        let cross = n1.x.native * n2.y.native - n1.y.native * n2.x.native
        let dot = n1.x.native * n2.x.native + n1.y.native * n2.y.native
        ...
        let dist = radius.native / tanHalfAngle

        // Start point of arc
        let arcStart = CGPoint(
            x: tangent1End.x - CGFloat(dist) * n1.x,
            y: tangent1End.y - CGFloat(dist) * n1.y
        )
        ...
        let perpDir = cross > 0 ? 1.0 : -1.0
        let perpX = CGFloat(-n1.y.native * perpDir)
        let perpY = CGFloat(n1.x.native * perpDir)
        let center = CGPoint(
            x: arcStart.x + perpX * radius,
            y: arcStart.y + perpY * radius
        )

        let startAngle = atan2((arcStart.y - center.y).native, (arcStart.x - center.x).native)
